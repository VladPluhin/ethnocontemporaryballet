    ...
    <Layout>
      <section className="section-content">
        <div className="container">
          <article>
            {eventData?.newEvent && (
              <a href={eventData.linkEvent} className="event-new">
                {eventData.newEventsText}
              </a>
            )}
            {eventData?.image?.url && !eventData?.projectgallery && (
              <div className="section-content__image">
                <img
                  ...
                  alt={eventData?.image?.description}
                />
              </div>
            )}
            ...
            {eventData.title && (
              <h1 className="section-content__title">{eventData.title}</h1>
            )}

            {eventData?.previewText && (
              <h4 className="section-content__subtitle">
                {eventData.previewText}
              </h4>
            )}
            <div className="section-content__content">
              {eventData.descriptionEvent && (
                <>{renderRichText(eventData.descriptionEvent)}</>
              )}
              {eventData.newEvent && eventData.linkEvent && (
                <div className="section-content__btn-wrap">
                  <a href={eventData.linkEvent} className="link ">
                    {eventData.textBtn}
                  </a>
                </div>
              )}
          ...
          <ul className="pagination">
            <li>
              {prevPage.slug && (
                <Link to={`/projects/${prevPage.slug}`} className="link prev ">
                  {prevPage.nameEvent}
                </Link>
              )}
            </li>
            <li>
              {nextPage.slug && (
                <Link to={`/projects/${nextPage.slug}`} className="link  next">
                  {nextPage.nameEvent}
                </Link>
              )}
            </li>
...
export const query = graphql`
  query MyQuery(
    ...
    previous: allContentfulCardEvent(
      filter: { slug: { eq: $previousPostSlug } }
    ) {
      nodes {
        slug
        nameEvent
